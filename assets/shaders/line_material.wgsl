...
// @group(1) @binding(0) var<uniform> world_from_local: mat3x4<f32>;
...
fn vertex(vertex: VertexInput) -> VertexOutput {
    ...
    let position = positions[vertex.index % 6];

    let world_from_local = get_world_from_local(vertex.instance_index); // affine3_to_square(material.world_from_local);

    // algorithm based on https://wwwtyro.net/2019/11/18/instanced-lines.html
    var clip_a = mesh_position_local_to_clip(world_from_local, vec4<f32>(vertex.position_a, 1.0));
    var clip_b = mesh_position_local_to_clip(world_from_local, vec4<f32>(vertex.position_b, 1.0));
    ...
    var line_width = material.line_width;
    var alpha = 1.;
    ...
    uv = (clipped_offset + position.y * world_distance) * resolution.y / near_clipping_plane_height / material.line_width;
#else
    ...
    uv = position.y * depth_adaptment * length(screen_b - screen_a) / material.line_width;
#endif
    ...
    var depth: f32;
    if material.depth_bias >= 0. {
        depth = clip.z * (1. - material.depth_bias);
    } else {
        ...
        depth = clip.z * exp2(-material.depth_bias * log2(clip.w / clip.z - EPSILON));
    }

    var clip_position = vec4(clip.w * ((2. * screen) / resolution - 1.), depth, clip.w);

    let line_fraction = 2.0 * material.line_scale / (material.gap_scale + material.line_scale);
    uv /= (material.gap_scale + material.line_scale) / 2.0;
...
fn fragment(in: VertexOutput) -> FragmentOutput {
    return FragmentOutput(in.color * material.color);
}
